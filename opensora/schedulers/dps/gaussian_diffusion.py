...
class GaussianDiffusion:
    ...
    def p_sample_loop_progressive(
        ...
        print("diffusion input size", img.shape)
        if progress:
        ...
        for i in indices:
            ...
            out = self.p_sample(model, img, t, clip_denoised=clip_denoised, denoised_fn=denoised_fn, cond_fn=cond_fn,
                                model_kwargs=model_kwargs)
            if observation is not None :
                observation = observation.to(device)
                x0_hat = out["pred_xstart"]
                ax0 = operator(x0_hat).to(device)
                difference = observation - ax0
                norm = th.linalg.norm(difference)
                norm_gard = th.autograd.grad(outputs=norm, inputs=img)[0]
                out["sample"] = out["sample"] - norm_gard * scale
                img = out["sample"]
            else:
                img = out["sample"]
            img = img.detach_()
